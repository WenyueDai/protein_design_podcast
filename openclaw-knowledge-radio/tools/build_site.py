...
def _script_url_from_audio_url(audio_url: str, date: str) -> str:
    """Derive the transcript .txt download URL from the MP3 release URL."""
    if not audio_url or "github.com" not in audio_url:
        return ""
    base = audio_url.rsplit("/", 1)[0]
    return f"{base}/podcast_script_{date}_llm_clean.txt"


def _format_transcript_html(text: str) -> str:
    """
    Convert raw script text to readable HTML for the transcript panel.
    [[TRANSITION]] markers become visible section dividers.
    """
    import re as _re
    sections = text.split("[[TRANSITION]]")
    parts = []
    for i, sec in enumerate(sections, 1):
        sec = sec.strip()
        if not sec:
            continue
        escaped = html.escape(sec)
        # Preserve line breaks
        escaped = escaped.replace("\n\n", "</p><p>").replace("\n", "<br>")
        label = f'<span class="tr-section-label">Section {i}</span>'
        parts.append(f'<div class="tr-section">{label}<p>{escaped}</p></div>')
    return "\n".join(parts)
...
def discover_episodes():
    ...
    for date, audio_url in release_idx.items():
        ...
        episodes_by_date[date] = {
            ...
            "script": None,
            "script_name": None,
            "script_url": _script_url_from_audio_url(audio_url, date),
            "script_text": None,
            "highlights": [],
            "items": [],
            "timestamps": [],
    ...
    if BASE_OUTPUT.exists():
        for d in BASE_OUTPUT.iterdir():
            ...
            _audio_url = release_idx.get(date, f"audio/podcast_{date}.mp3")
            ep = episodes_by_date.setdefault(date, {
                ...
                "script": None,
                "script_name": None,
                "script_url": _script_url_from_audio_url(_audio_url, date),
                "script_text": None,
                "highlights": [],
                "items": [],
                "timestamps": [],
            ...
            if script_path:
                ...
                try:
                    ep["script_text"] = script_path.read_text(encoding="utf-8", errors="ignore")
                except Exception:
                    pass
            if episode_items:
...
.neko-leg-l,.neko-leg-r {{ transform-box:fill-box; transform-origin:top center; }}
.neko-bowl,.neko-zzz,.neko-legs {{ display:none; }}
...
#ghibli-cat.walking .neko-leg-l {{ animation:neko-walk-a .42s ease-in-out infinite; }}
#ghibli-cat.walking .neko-leg-r {{ animation:neko-walk-b .42s ease-in-out infinite; }}
...
@keyframes neko-walk-b {{ 0%,100% {{ transform:rotate(24deg); }} 50% {{ transform:rotate(-24deg); }} }}
@keyframes neko-bob {{ 0%,50%,100% {{ transform:translateY(0); }} 25%,75% {{ transform:translateY(-2px); }} }}
...
<!-- ── Cat: front-view (eat/walk/sleep/read) ── -->
<div id="ghibli-cat">
...
(function() {{
  ...
  if (!cat) return;
  var x, y, tx, ty, state, stateTimer, facingLeft, speed = 1.6;
  ...
  function tick() {{
    if (state === 'walking') {{
      ...
      if (--stateTimer <= 0) {{
        var r = Math.random();
        if      (r < .18) {{ goEating(); }}
        else if (r < .28) {{ goReading(); }}
        else if (r < .72) {{ newTarget(); state='walking'; facingLeft=tx<x; cls(); }}
        else               {{ rest(); }}
      }}
    }}
